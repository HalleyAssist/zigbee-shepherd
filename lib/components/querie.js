...
const indirectTimeout = 8000
const completeTimeout = 55000
...
querie.device = async function (ieeeAddr, nwkAddr) {
    ...
    let rsp = await controller.limitConcurrency(
        async()=>{
            return await controller.indirectRequestSend({completeTimeout, indirectTimeout, dstAddr:nwkAddr}, 'ZDO', 'nodeDescReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr })
        },
        ieeeAddr
    )(true)
    ...
    rsp = await controller.limitConcurrency(
        async()=>{
            return await controller.indirectRequestSend({completeTimeout, indirectTimeout, dstAddr:nwkAddr}, 'ZDO', 'activeEpReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr })
        },
        ieeeAddr
    )(true)
...
querie.endpoint = async function (ieeeAddr, nwkAddr, epId) {
    ...
    const rsp = await controller.limitConcurrency(
        async()=>{
            return await controller.indirectRequestSend({completeTimeout, indirectTimeout, dstAddr:nwkAddr}, 'ZDO', 'simpleDescReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr, endpoint: epId })
        },
        ieeeAddr
    )(true)
