...
class Controller extends EventEmitter {
    ...
    async _queryBasic(dev, aborted){
        ...
        try {
            readStatusRecsRsp = await retry(() => { 
                if(aborted()) throw new Error("__abort__")
                return this.limitConcurrency(function(){return this._shepherd.af.zclFoundation(basicEpInst, basicEpInst, 0, 'read', [{ attrId: 4 }, { attrId: 5 }, { attrId: 7 }]) })
            }, 3)
        } catch(err){
    ...
    async _fullInterview(dev, aborted){
        ...
        _.forEach(dev.endpoints, (ep) => {
            clustersReqs.push(async () => {
                if(aborted()) throw new Error("__abort__")
                const clusters = await shepherd.af.zclClustersReq(ep, interviewEvents, interested)
                if(aborted()) throw new Error("__abort__")
    ...
    async _endDeviceAnnceHdlr (data, aborted) {
        ...
        try {
            ...
            if(dev.incomplete) {
                ...
                await this._fullInterview(dev, aborted)
            }
