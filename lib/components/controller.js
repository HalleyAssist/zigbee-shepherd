        ...
        if (msg.hasOwnProperty('endpoint')) {                                               // dataConfirm, reflectError
            ...
            if (targetEp) {  // local
                ...
                if (!remoteEp) {        // local zApp not found, get ieeeaddr and emit fake 'endDeviceAnnceInd' msg
                    let dev = await this._shepherd.findDevByAddr(msg.srcaddr)
                    ...
                    if(!dev){
                        ...
                        dev = await this._shepherd.findDevByAddr(rsp.ieeeaddr)
                        if(dev){
                            ...
                            if (Array.isArray(msgBuffer)) {
                                ...
                                this.joinHandler({ srcaddr: rsp.nwkaddr, nwkaddr: rsp.nwkaddr, ieeeaddr: rsp.ieeeaddr }, false)
                            }
