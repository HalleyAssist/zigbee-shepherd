...
Controller.prototype.reset = function (mode, callback) {
    ...
    function resetListener(err) {
        return err ? deferred.reject(err) : deferred.resolve();
    }
    this.once('_reset', resetListener);
    ...
    return Q.fcall(function () {
        ...
        setTimeout(function(){
            ...
            self.removeListener('SYS:resetInd', resetListener);
        }, 1000)
...
Controller.prototype._indirect_send = function(dstAddr, _send, promise){
    ...
    function handleResend(data){
        ...
        if(data.code != 6) {
            self.once(afResendEvt, handleResend);
            return
        }
        if((Date.now() - sendTime) > 6900){
    ...
    function handleAps(){   
        ...
        if((Date.now() - initialTime) < 18000){
            debug.shepherd("possible indirect expiration due to APS timeout, resending")
            self.removeListener(afResendEvt, handleResend)
            send()
            return true
        }
        return false
    ...
    function send(){
        ...
        if(ret && ret.then){
            ret.done(deferred.resolve, deferred.reject)
        }
    }
    ...
    ret = {evt: afResendEvt, fn: handleResend, result: deferred.promise, done: done, isDone: false, handleAps: handleAps}
    return ret
}
