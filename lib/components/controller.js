...
Controller.prototype.request = async function (subsys, cmdId, valObj, callback) {
    var deferred = Q.defer(),
        rspHdlr;
    ...
    if (subsys === 'ZDO' || subsys === 5)
        ...
        try {
            const rsp = await this._znp.request(subsys, cmdId, valObj);  // SREQ has timeout inside znp
            rspHdlr(null, rsp)
        } catch(ex){
            rspHdlr(ex, null)
        }
