...
class Controller extends AfController {
    ...
    async checkNvParams () {
        ...
        const steps = [
            ...
            //['nwkkey', 'SYS', 'osalNvRead', nvParams.nwkkey],
        ...
        try {
            for(const step of steps){
                ...
                const nv = nvParams[key]
                if (!_.isEqual(bufToArray(rsp.value.slice(0, nv.len)), nv.value)) throw new Error(`reset ${key}`)
            }
        } catch(ex){
            let err = ex
        ...
        if (msg.hasOwnProperty('endpoint')) {                                               // dataConfirm, reflectError
            ...
            if (targetEp) {  // local
                ...
                if (!remoteEp) {        // local zApp not found, get ieeeaddr and emit fake 'endDeviceAnnceInd' msg
                    ...
                    const dev = await this._shepherd.findDevByAddr(msg.srcaddr)
                    if(dev) remoteEp = new Endpoint(dev, {epId: msg.srcendpoint}, this._shepherd)
                    const deferred = Q.defer()

                    if (Array.isArray(msgBuffer)) {
                        if(!dev) msgBuffer.push({ type: type, msg: msg });
                    } else if (msgBuffer === undefined) {
                        if(!dev) msgBuffer = this._rebornDevs[msg.srcaddr] = [ { targetEp, type, msg, deferred } ];

                        try {
                            const rsp = await this.request('ZDO', 'ieeeAddrReq', { shortaddr: msg.srcaddr, reqtype: 0, startindex:0 })
    ...
    async _endDeviceAnnceHdlr (data, aborted) {
        var joinTimeout,
            ...
            dev = await this._shepherd.findDevByAddr(data.ieeeaddr);
        ...
        if(dev){
            ...
            dev = new Device(this._shepherd.devstore, {ieeeAddr: data.ieeeaddr, nwkAddr: data.nwkaddr}, this._shepherd);    
            await dev.insert()
        }
        ...
        try {
            ...
            /* Verdict */
            await dev.update({ status: 'online', complete:true })
    ...
    _indirect_send (dstAddr, _send, promise){
        let sendTime, deferred = Q.defer()
        const afResendEvt = 'ZDO:networkStatus:' + dstAddr
        let ret, initialTime = Date.now()
        let eventArgs
        ...
        let timeoutMax = Math.ceil(this._shepherd.af.resendTimeout / 8000)
        const handleTimeout = ()=>{   
            if(promise && promise.isPending && !promise.isPending()) {
                ret.isDone = true
                return false
            ...
            if(!--timeoutMax){
                debug.shepherd(`possible indirect expiration due to timeout, but exeeded max count`)
                return false
            }
            if((Date.now() - initialTime) < this._shepherd.af.resendTimeout){
        ...
        ret = {evt: afResendEvt, result: deferred.promise, done, isDone: false, handleTimeout}
        send()

        return ret
    }

    async indirect_send (dstAddr, _send, promise){
        let ret = this._indirect_send(dstAddr, _send, promise)
