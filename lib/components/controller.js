            ...
            if (targetEp) {  // local
                ...
                if (!remoteEp) {        // local zApp not found, get ieeeaddr and emit fake 'endDeviceAnnceInd' msg
                    ...
                    if (Array.isArray(msgBuffer)) {
                        ...
                        if(!dev) msgBuffer = this._rebornDevs[msg.srcaddr] = [ { type, msg, deferred } ];
                            ...
                            this.once('ind:incoming:' + rsp.ieeeaddr, () => {
                                if (this.findEndpoint(msg.srcaddr, msg.srcendpoint) && Array.isArray(msgBuffer))
                                    for(const item of msgBuffer) {
                                        af.dispatchIncomingMsg(item.type, item.msg).then(item.deferred.resolve, item.deferred.reject);
                                    }
                                else
