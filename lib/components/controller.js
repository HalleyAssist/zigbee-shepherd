...
class Controller extends AfController {
    ...
    async permitJoin(duration, type) {
        // duration: seconds, 0x00 disable, 0xFF always enable
        // type: 0 (coord) / 1 (all)

        proving.number(duration, 'time should be a number.');
        ...
        const rsp = await this.request('ZDO', 'mgmtPermitJoinReq', { addrmode, dstaddr, duration, tcsignificance: 0 });

        this._shepherd.emit('permitJoining', {duration, type});
            ...
            if (targetEp) {  // local
                ...
                if (!remoteEp) {        // local zApp not found, get ieeeaddr and emit fake 'endDeviceAnnceInd' msg
                    ...
                    if (!dev) {
                        ...
                        if (dev) {
                            debug.shepherd(`updating network for ${rsp.ieeeaddr} (0x${dev.nwkAddr.toString(16)}) to 0x${msg.srcaddr.toString(16)} with altNwk adjustment`)
                            ...
                            if (msgBuffer) {
                                ...
                                this.joinHandler({ srcaddr: rsp.nwkaddr, nwkaddr: rsp.nwkaddr, ieeeaddr: rsp.ieeeaddr }, false).then(dev=>{
    ...
    async _endDeviceAnnceHdlr(data, aborted) {
        ...
        try {
            ...
            if (!info.endpoints.length) {
                throw new Error('No device endpoints')
            }
