...
function generateEvent(indName, meta, valObj){
    ...
    var evtName = 'ZDO:' + indName;

    meta.forEach(function (key) {
        evtName += ':' + valObj[key].toString();
    });
...
zdoHelper.generateDstAddrOfRequest = function(valObj){
    if(valObj.dstaddr) return valObj.dstaddr.toString()
    if(valObj.nwkaddrofinterest) return valObj.nwkaddrofinterest.toString()
};
