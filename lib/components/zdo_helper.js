...
function generateEvent(indName, meta, valObj){
    if (!meta || (meta.length === 0))
        return;

    let evtName = 'ZDO:' + indName;

    for(const key of meta) {
        if(key === 'shortaddr' || key === 'srcaddr' || key === 'nwkaddr' || key === 'nwkaddrofinterest' || key === 'dstaddr') evtName += ':' + valObj[key].toString(16)
        else evtName += ':' + valObj[key].toString()
    }
...
zdoHelper.generateEventOfRequest = function (reqName, valObj) {
    ...
    return generateEvent(meta.ind, meta.suffix, valObj)
};

zdoHelper.generateEventOfIndication = function (indName, msgData) {
    return generateEvent(indName, zdoIndSuffix[indName], msgData)
};

zdoHelper.generateDstAddrOfRequest = function(valObj){
    if(valObj.dstaddr) return valObj.dstaddr.toString(16)
    if(valObj.nwkaddrofinterest) return valObj.nwkaddrofinterest.toString(16)
};
