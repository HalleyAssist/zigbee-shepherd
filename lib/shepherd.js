...
ZShepherd.prototype.mount = async function (zApp) {
    var coord = this.controller.getCoord(),
        mountId,
        loEp;

    assert (zApp.constructor.name === 'Zive', 'zApp should be an instance of Zive class.')
    ...
    try {
        ...
        await Q.ninvoke(this._devbox, 'sync', coord._getId());
...
ZShepherd.prototype._unregisterDev = async function (dev) {
    return await this.clearDev(dev._getId());
};

ZShepherd.prototype._attachZclMethods = function (ep) {
    if (ep.constructor.name === 'Zive') {
        var zApp = ep;
        zApp.foundation = (dstAddr, dstEpId, cId, cmd, zclData, cfg) => {
            var dstEp = this.find(dstAddr, dstEpId);

            if (!dstEp)
                throw new Error('dstEp is not found.')
                
            return this._foundation(zApp._endpoint, dstEp, cId, cmd, zclData, cfg);
        };

        zApp.functional = (dstAddr, dstEpId, cId, cmd, zclData, cfg) => {
            var dstEp = this.find(dstAddr, dstEpId);

            if (!dstEp)
                throw new Error('dstEp is not found.')
            
            return this._functional(zApp._endpoint, dstEp, cId, cmd, zclData, cfg);
        };
    } else {
        ep.foundation = (cId, cmd, zclData, cfg) => {
            return this._foundation(ep, ep, cId, cmd, zclData, cfg);
        };
        ep.functional = (cId, cmd, zclData, cfg) => {
            return this._functional(ep, ep, cId, cmd, zclData, cfg);
        };
        ...
        ep.read = async (cId, attrId) => {
            ...
            if (rec.status !== 0) throw new ZigbeeError('request unsuccess: ' + rec.status, rec.status);
            return rec.attrData;
...
ZShepherd.prototype._updateFinalizer = async function (ep, cId, attrs, reported) {
    ...
    try {
        if (attrs) {
            ...
            newAttrs = await this.af._zclClusterAttrsReq(ep, cId);
        }
    } catch(ex){
        return
    }
