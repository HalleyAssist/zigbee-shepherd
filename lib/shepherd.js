...
ZShepherd.prototype._attachZclMethods = function (ep, isZive = false) {
    ...
    if (isZive) {
        ep.foundation = async (argShepherd, dstAddr, dstEpId, cId, cmd, zclData, cfg) => {
            if(argShepherd && argShepherd !== this) throw new Error('Supplied unexpected shepherd')
        ...
        ep.functional = async (argShepherd, dstAddr, dstEpId, cId, cmd, zclData, cfg) => {
            if(argShepherd && argShepherd !== this) throw new Error('Supplied unexpected shepherd')
            var dstEp = await this.find(dstAddr, dstEpId);
