...
function ZShepherd(sp, opts, keystore) {
    ...
    this.devstore = keystore.section("devices")
...
ZShepherd.prototype.list = async function (ieeeAddrs, showIncomplete = false) {
    ...
    let foundDevs = []
    for(const dev of await Device.all(this.devstore, this)){
...
ZShepherd.prototype.find = async function (addr, epId) {
    proving.number(epId, 'epId should be a number.');

    var dev = await this.findDevByAddr(addr);
...
ZShepherd.prototype.rtg = async function (ieeeAddr) {
    ...
    var dev = await this.findDevByAddr(ieeeAddr);
...
/*** Protected Methods                                                                         ***/
/*************************************************************************************************/
ZShepherd.prototype.findDevByAddr = async function (addr) {
    // addr: ieeeAddr(String) or nwkAddr(Number)
    proving.stringOrNumber(addr, 'addr should be a number or a string.');

    if(typeof addr === 'string'){
        return await Device.get(addr,this.devstore,this)
    }

    const devices = await Device.all(this.devstore, this)
...
ZShepherd.prototype._attachZclMethods = function (ep, isZive = false) {
    ...
    if (isZive) {
        ep.foundation = async (dstAddr, dstEpId, cId, cmd, zclData, cfg) => {
            var dstEp = await this.find(dstAddr, dstEpId);
        ...
        ep.functional = async (dstAddr, dstEpId, cId, cmd, zclData, cfg) => {
            var dstEp = await this.find(dstAddr, dstEpId);
...
ZShepherd.prototype.interestedEndpoint = function(endpoint, basicData){
    // Can be overridden
    return true
}
