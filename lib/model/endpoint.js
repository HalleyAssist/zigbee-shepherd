...
Endpoint.prototype.isZive = ()=>false


Endpoint.prototype.foundation = async function(shepherd, cId, cmd, zclData, cfg) {
    this._logger(`zcl:foundation(${cmd}) ${JSON.stringify(zclData)}`)
    return await shepherd._foundation(null, this, cId, cmd, zclData, cfg);
};
Endpoint.prototype.functional = async function(shepherd, cId, cmd, zclData, cfg) {
    this._logger(`zcl:functional(${cmd}) ${JSON.stringify(zclData)}`)
    return await shepherd._functional(null, this, cId, cmd, zclData, cfg);
};
Endpoint.prototype.bind = async function(shepherd, cId, dstEpOrGrpId) {
    return await shepherd.controller.bind(this, cId, dstEpOrGrpId);
};
Endpoint.prototype.unbind = async function(shepherd, cId, dstEpOrGrpId) {
    return await shepherd.controller.unbind(this, cId, dstEpOrGrpId);
};
Endpoint.prototype.read = async function (shepherd, cId, attrId) {
    var attr = zclId.attr(cId, attrId);

    attr = attr ? attr.value : attrId;

    const readStatusRecsRsp = await shepherd._foundation(this, this, cId, 'read', [{ attrId: attr }])
...
Endpoint.prototype.write = async function (shepherd, cId, attrId, data) {
    var attr = zclId.attr(cId, attrId),
        attrType = zclId.attrType(cId, attrId).value;

    const writeStatusRecsRsp = await shepherd._foundation(this, this, cId, 'write', [{ attrId: attr.value, dataType: attrType, attrData: data }])
...
Endpoint.prototype.report = async function (shepherd, cId, attrId, minInt, maxInt, repChange) {
    let coord = shepherd.controller.getCoord(),
    ...
    const rsp = await this.bind(shepherd, cId, dlgEp)
    if (cfgRpt){
        const rsp = await this.foundation(shepherd, cId, 'configReport', [ cfgRptRec ])
