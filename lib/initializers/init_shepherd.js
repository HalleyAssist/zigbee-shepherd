...
init.setupShepherd = function (shepherd) {
    var controller = shepherd.controller,
        netInfo;

    debug('zigbee-shepherd booting...');

    shepherd.af = new Af(controller);
    
    // attach event listeners
    for(const rec of Af.msgHandlers){
        controller.on(rec.evt, (msg)=>controller._afDispatch(shepherd.af, rec.hdlr, msg));
    }

    shepherd.af.on('all', ({eventName,args})=>{
